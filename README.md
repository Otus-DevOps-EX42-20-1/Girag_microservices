...
- Развёрнут `Prometheus` в кластере `Kubernetes`.
- В нём также настроены `Prometheus` и `Grafana` для сбора метрик.
- С помощью стека `EFK` выстроен процесс сбора и агрегации логов, их хранения, поиска и отображения результатов.
